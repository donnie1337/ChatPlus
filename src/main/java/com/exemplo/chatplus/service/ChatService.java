...
public final class ChatService {
    ...
    private boolean isAuthenticated(Player player) {
        ...
        if (auth == null || !auth.isEnabled()) { authPlugin = null; authCheckMethod = null; return false; } Method method = authCheckMethod;
    ...
    private boolean isVanished(Player player) {
        ...
        if (vanish == null || !vanish.isEnabled()) { vanishPlugin = null; vanishCheckMethod = null; return false; } Method method = vanishCheckMethod;
    ...
    private BaseComponent[] formatMessage(String format, CommandSender sender, String message, String world, Player viewer, boolean senderVanished, String chatType) {
        ...
        if (sender instanceof ConsoleCommandSender) playerName = "Console"; else {
            Player player = (Player) sender; prefix = cargo.getPrefix(player.getUniqueId()); cargoColor = cargo.getNicknameColor(player.getUniqueId()); if (cargoColor.isEmpty()) cargoColor = firstColorCode(prefix); if (cargoColor.isEmpty()) cargoColor = "§f"; playerName = cargoColor + sender.getName(); clanTag = cargo.getClanTag(player.getUniqueId());
            if (senderVanished && viewer != null && viewer.hasPermission(VANISH_PERMISSION)) { playerName += " " + VANISH_SUFFIX_MARKER; addVanishHover = true; } chatColor = cargo.getChatColor(player.getUniqueId()); group = cargo.getGroup(player.getUniqueId());
        }
        Map<String, String> placeholders = new HashMap<>(); placeholders.put(PLAYER_PLACEHOLDER, playerName); placeholders.put(TAG_PLACEHOLDER, clanTag); placeholders.put("{message}", chatColor + MessageUtil.sanitizePlayerText(message)); placeholders.put("{world}", world); String template = MessageUtil.colorize(format == null ? "" : format);
        if (sender instanceof Player && template.contains(PREFIX_PLACEHOLDER)) {
            String before = template.substring(0, template.indexOf(PREFIX_PLACEHOLDER)); String afterTemplate = template.substring(template.indexOf(PREFIX_PLACEHOLDER) + PREFIX_PLACEHOLDER.length()); placeholders.put(PREFIX_PLACEHOLDER, ""); before = MessageUtil.apply(before, placeholders); afterTemplate = afterTemplate.replace(TAG_PLACEHOLDER, ""); List<BaseComponent> components = new ArrayList<>(); addLegacy(components, before);
            BaseComponent[] prefixComponents = TextComponent.fromLegacyText(MessageUtil.colorize(prefix)); HoverEvent prefixHover = buildCargoHover(cargoColor, group); for (BaseComponent component : prefixComponents) { component.setHoverEvent(prefixHover); components.add(component); }
            if (!clanTag.isEmpty()) addClanTag(components, clanTag, cargoColor); addPlayerAndAfter(components, afterTemplate, (Player) sender, playerName, cargoColor, addVanishHover, placeholders); BaseComponent[] result = components.toArray(BaseComponent[]::new); decorateChatTypeHover(result, chatType); return result;
        }
    ...
    private HoverEvent buildCargoHover(String cargoColor, String group) { String safeColor = cargoColor == null || cargoColor.isEmpty() ? "§f" : cargoColor; String cargoName = capitalizeGroupName(group); return new HoverEvent(HoverEvent.Action.SHOW_TEXT, new ComponentBuilder("§fCargo: ").append(TextComponent.fromLegacyText(safeColor + cargoName)).create()); }
    ...
    private void addPlayerAndAfter(List<BaseComponent> components, String template, Player player, String playerName, String cargoColor, boolean addVanishHover, Map<String, String> placeholders) {
        int playerIndex = template.indexOf(PLAYER_PLACEHOLDER); if (playerIndex < 0) { placeholders.put(PLAYER_PLACEHOLDER, playerName); if (addVanishHover) addLegacyWithVanishHover(components, MessageUtil.apply(template, placeholders)); else addLegacy(components, MessageUtil.apply(template, placeholders)); return; }
        String beforePlayer = template.substring(0, playerIndex); String afterPlayer = template.substring(playerIndex + PLAYER_PLACEHOLDER.length()); placeholders.put(PLAYER_PLACEHOLDER, ""); addLegacy(components, MessageUtil.apply(beforePlayer, placeholders)); String nickname = cargoColor + player.getName();
        HoverEvent nicknameHover = new HoverEvent(HoverEvent.Action.SHOW_TEXT, new ComponentBuilder(buildPlayerProfileHover(player, nickname, cargoColor)).create()); BaseComponent[] nicknameComponents = TextComponent.fromLegacyText(nickname);
        for (BaseComponent component : nicknameComponents) { component.setHoverEvent(nicknameHover); component.setClickEvent(new ClickEvent(ClickEvent.Action.SUGGEST_COMMAND, player.getName())); components.add(component); }
    ...
    private String buildPlayerProfileHover(Player player, String nickname, String cargoColor) {
        String cargoValue = cargo.getGroup(player.getUniqueId()); if (cargoValue == null || cargoValue.isBlank()) cargoValue = "Desconhecido"; String clanValue = cargo.getClanTag(player.getUniqueId()); boolean hasClan = clanValue != null && !clanValue.isBlank(); if (!hasClan) clanValue = "Nenhum"; else clanValue = MessageUtil.colorize(clanValue);
        String moneyValue = getMoney(player); int kills = player.getStatistic(Statistic.PLAYER_KILLS); int deaths = player.getStatistic(Statistic.DEATHS); double kdr = deaths <= 0 ? kills : (double) kills / deaths; String safeCargoColor = cargoColor == null || cargoColor.isEmpty() ? "§f" : cargoColor;
        StringBuilder lore = new StringBuilder(); lore.append("ㅤ§f§l◆ INFORMAÇÕES ◆ㅤ\n"); lore.append("ㅤ\n"); lore.append("ㅤ  §7ᴊᴏɢᴀᴅᴏʀ §8• ").append(safeCargoColor).append(nickname.replaceFirst("§[0-9a-fk-or]", "")).append("\n"); lore.append("ㅤ  §7ᴄᴀʀɢᴏ §8• ").append(safeCargoColor).append(capitalizeGroupName(cargoValue)).append("\n");
    ...
    private String getMoney(Player player) {
        try { Class<?> economyClass = Class.forName("net.milkbowl.vault.economy.Economy"); Object registration = Bukkit.getServicesManager().getRegistration(economyClass); if (registration == null) return "0,00"; Object provider = economyClass.cast(((org.bukkit.plugin.RegisteredServiceProvider<?>) registration).getProvider()); Method balanceMethod = economyClass.getMethod("getBalance", OfflinePlayer.class); Object balance = balanceMethod.invoke(provider, player); if (balance instanceof Number number) { DecimalFormatSymbols symbols = DecimalFormatSymbols.getInstance(new Locale("pt", "BR")); DecimalFormat format = new DecimalFormat("#,##0.00", symbols); return format.format(number.doubleValue()); } } catch (ReflectiveOperationException | LinkageError | RuntimeException ignored) { } return "0,00";
    }

    private String formatOnlineTime(Player player) { long minutes = player.getStatistic(Statistic.PLAY_ONE_MINUTE) / 20L / 60L; long days = minutes / (24L * 60L); long hours = (minutes % (24L * 60L)) / 60L; long mins = minutes % 60L; if (days > 0) return days + "d " + hours + "h " + mins + "m"; if (hours > 0) return hours + "h " + mins + "m"; return mins + "m"; }
    private String capitalizeGroupName(String group) { if (group == null || group.isBlank()) return "Desconhecido"; return group.substring(0, 1).toUpperCase(Locale.ROOT) + group.substring(1).toLowerCase(Locale.ROOT); }
    private String firstColorCode(String text) { if (text == null) return ""; for (int i = 0; i < text.length() - 1; i++) if (text.charAt(i) == '§') return text.substring(i, i + 2); return ""; }
    private void addLegacy(List<BaseComponent> components, String text) { if (text == null || text.isEmpty()) return; for (BaseComponent component : TextComponent.fromLegacyText(text)) components.add(component); }
    private void addLegacyWithVanishHover(List<BaseComponent> components, String text) { if (text == null || text.isEmpty()) return; BaseComponent[] parsed = TextComponent.fromLegacyText(text); HoverEvent hover = new HoverEvent(HoverEvent.Action.SHOW_TEXT, TextComponent.fromLegacyText(VANISH_HOVER_TEXT)); for (BaseComponent component : parsed) { if (component.toPlainText().contains(VANISH_SUFFIX_MARKER)) { component.setText(component.toPlainText().replace(VANISH_SUFFIX_MARKER, VANISH_SUFFIX)); component.setHoverEvent(hover); } components.add(component); } }
    private BaseComponent[] parseWithVanishHover(String text) { List<BaseComponent> components = new ArrayList<>(); addLegacyWithVanishHover(components, text); return components.toArray(BaseComponent[]::new); }
    private void addClanTag(List<BaseComponent> components, String clanTag, String cargoColor) { BaseComponent[] tag = TextComponent.fromLegacyText("§8[" + MessageUtil.colorize(clanTag) + "§8]"); for (BaseComponent component : tag) { component.setHoverEvent(buildCargoHover(cargoColor, cargo.getGroup(clanTag.hashCode() > 0 ? new java.util.UUID(0L, 0L) : new java.util.UUID(0L, 0L)))); components.add(component); } }
